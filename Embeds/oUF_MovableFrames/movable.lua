...
local _DB
local _DBNAME = GetAddOnMetadata(_NAME, 'X-MoveDB')
...
local restoreDefaultPosition = function(style, identifier)
	...
	if(obj) then
		...
		local point, parentName, x, y, scale = 
			_DB.__INITIAL[style][identifier].point,
			_DB.__INITIAL[style][identifier].parent,
			_DB.__INITIAL[style][identifier].x,
			_DB.__INITIAL[style][identifier].y,
			_DB.__INITIAL[style][identifier].scale
...
local function restorePosition(obj)
	...
	local point, parentName, x, y, scale = 
		_DB[style][identifier].point,
		_DB[style][identifier].parent,
		_DB[style][identifier].x,
		_DB[style][identifier].y,
		_DB[style][identifier].scale
...
do
	...
	opt:SetScript("OnShow", function(self)
		...
		anchors:SetSize(150, 22)
		anchors:SetText("Toggle Anchors")
		anchors:SetScript("OnClick", ToggleAnchors)
		
		local scroll = CreateFrame("ScrollFrame", nil, self)
		scroll:SetPoint('TOPLEFT', anchors, 'BOTTOMLEFT', 0, -8)
		...
		function createOrUpdate()
			...
			for unit, points in next, styleData do
				...
				row.scaleLabel:SetText(string.format('%.2fx', points.scale or 1))
				row.xLabel:SetText(points.x)
				row.yLabel:SetText(points.y)
				row.pointLabel:SetText(points.point)
...
local slashList = GetAddOnMetadata(_NAME, 'X-MoveCmd'):gsub('%s+', '')
