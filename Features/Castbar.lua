...
local colorYellow = colors.reaction[4]
local colorRed = colors.reaction[1]
------------------------------------------------------------------------
...
function prototype:PostCastStart(unit, name, rank, castid)
	...
	local color = ns.config.castColor
	local castBG = ns.config.castBG
	...
	self.bg:SetVertexColor(r * castBG, g * castBG, b * castBG)
